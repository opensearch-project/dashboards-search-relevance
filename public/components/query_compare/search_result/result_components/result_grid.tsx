...
export const ResultGridComponent = ({
  ...
  const applyHighlighting = (text: string, fieldName: string, highlights?: { [key: string]: string[] }) => {
    if (!highlights || !highlights[fieldName]) {
      return text;
    }
    
    let highlightedText = text;
    highlights[fieldName].forEach(highlight => {
      const emMatches = highlight.match(/<em>(.*?)<\/em>/g);
      if (emMatches) {
        emMatches.forEach(match => {
          const cleanText = match.replace(/<\/?em>/g, '');
          highlightedText = highlightedText.replace(cleanText, `<mark style="background-color: yellow;">${cleanText}</mark>`);
        });
      }
    });
        ...
        >
          <EuiText size="m" className="eui-yScroll osdDocTableCell__detailsExpanded">
            {_.toPairs(docSource).map((entry: string[]) => {
              const fieldValue = _.isObject(entry[1]) ? JSON.stringify(entry[1]) : entry[1];
              const highlightedContent = typeof fieldValue === 'string'
                ? applyHighlighting(fieldValue, entry[0], queryResult.hits.hits.find(hit => hit._source === docSource)?.highlight)
                : fieldValue;
        ...
        <span>
          ...
          >
            {_.toPairs(sourceFields).map((entry: string[]) => {
              const fieldValue = _.isObject(entry[1]) ? JSON.stringify(entry[1]) : entry[1];
              const highlightedContent = typeof fieldValue === 'string' 
                ? applyHighlighting(fieldValue, entry[0], doc.highlight)
                : fieldValue;
              ...
              return (
                <span key={uniqueId('grid-dt-dd-')}>
                  ...
                  <EuiDescriptionListDescription style={{ wordBreak: 'break-word', whiteSpace: 'normal', width: '50%' }}>
                    {highlightedContent}
                  </EuiDescriptionListDescription>
                </span>
              );
  ...
  const getRankColumn = (documentId: string, documentRank: number) => {
    return (
      <td key={`${resultNumber}-${documentId}`}>
        <EuiFlexGroup style={{ width: '50%' }} direction="column" alignItems="flexStart">
          <EuiFlexItem>
            <EuiTitle size="xs">
              <h1
                style={{
